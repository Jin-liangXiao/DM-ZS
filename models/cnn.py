# from torch.nn import functional 
import torch
import torch.nn as nn
from .common import *
from torch.nn import functional





class CNN(nn.Module):
    def __init__(self,a,b):
        super(CNN, self).__init__()
        self.conv1 = nn.Sequential(        
            nn.Conv2d(a+b, 128-b, 3, 1, 1),     
            nn.LeakyReLU(negative_slope=0.2, inplace=False), 
            )
        self.conv2 = nn.Sequential(        
        nn.Conv2d(128, 128-b, 3, 1, 1),     
        nn.LeakyReLU(negative_slope=0.2, inplace=False), 
        )
        self.conv3 = nn.Sequential(        
        nn.Conv2d(128, 128-b, 3, 1, 1),     
        nn.LeakyReLU(negative_slope=0.2, inplace=False), 
        )
        self.conv4 = nn.Sequential(        
        nn.Conv2d(128, a, 3, 1, 1),     
        )

        basecoeff = torch.Tensor([[-4.63495665e-03, -3.63442646e-03,  3.84904063e-18,
                     5.76678319e-03,  1.08358664e-02,  1.01980790e-02,
                    -9.31747402e-18, -1.75033181e-02, -3.17660068e-02,
                    -2.84531643e-02,  1.85181518e-17,  4.42450253e-02,
                     7.71733386e-02,  6.70554910e-02, -2.85299239e-17,
                    -1.01548683e-01, -1.78708388e-01, -1.60004642e-01,
                     3.61741232e-17,  2.87940558e-01,  6.25431459e-01,
                     8.97067600e-01,  1.00107877e+00,  8.97067600e-01,
                     6.25431459e-01,  2.87940558e-01,  3.61741232e-17,
                    -1.60004642e-01, -1.78708388e-01, -1.01548683e-01,
                    -2.85299239e-17,  6.70554910e-02,  7.71733386e-02,
                     4.42450253e-02,  1.85181518e-17, -2.84531643e-02,
                    -3.17660068e-02, -1.75033181e-02, -9.31747402e-18,
                     1.01980790e-02,  1.08358664e-02,  5.76678319e-03,
                     3.84904063e-18, -3.63442646e-03, -4.63495665e-03]])
        coeff = torch.mm(basecoeff.T, basecoeff)
        coeff = torch.Tensor(coeff)
        coeff = torch.unsqueeze(coeff, 0)
        coeff = torch.unsqueeze(coeff, 0)
        self.coeff = torch.repeat_interleave(coeff, a,0)

    def forward(self, x,y,sf=6):
        def Upsample_4(coeff,inputs):
                    _,c, h, w= inputs.shape
                    # outs = functional.conv_transpose2d(inputs, coeff.cuda(), bias=None, stride=2, padding=21, output_padding=1, groups=c, dilation=1)
                    outs = functional.conv_transpose2d(inputs, coeff.cuda(), bias=None, stride=4, padding=21,
                                                       output_padding=1, groups=c, dilation=1)
                    return outs
        x1=Upsample_4(self.coeff,x)
        # x1=functional.interpolate(x, size=None, scale_factor=sf, mode='bilinear', align_corners=None)
        x2 = torch.cat((x1,y),1)
        x2 = torch.cat((self.conv1(x2),y),1)
        x2 = torch.cat((self.conv2(x2),y),1)
        x2 = torch.cat((self.conv3(x2),y),1)
        x3 = self.conv4(x2)
        return x3+x1


class ZSSRNet(nn.Module):
	def __init__(self, input_channels=3, kernel_size=3, channels=64):
		super(ZSSRNet, self).__init__()
		 
		self.conv0 = nn.Conv2d(input_channels, channels, kernel_size=kernel_size, padding=kernel_size//2, bias=True)
		self.conv1 = nn.Conv2d(channels, channels, kernel_size=kernel_size, padding=kernel_size//2, bias=True)
		self.conv2 = nn.Conv2d(channels, channels, kernel_size=kernel_size, padding=kernel_size//2, bias=True)
		self.conv3 = nn.Conv2d(channels, channels, kernel_size=kernel_size, padding=kernel_size//2, bias=True)
		self.conv4 = nn.Conv2d(channels, channels, kernel_size=kernel_size, padding=kernel_size//2, bias=True)
		self.conv5 = nn.Conv2d(channels, channels, kernel_size=kernel_size, padding=kernel_size//2, bias=True)
		self.conv6 = nn.Conv2d(channels, channels, kernel_size=kernel_size, padding=kernel_size//2, bias=True)
		self.conv7 = nn.Conv2d(channels, input_channels, kernel_size=kernel_size, padding=kernel_size//2, bias=True)

		self.relu = nn.ReLU()


	def forward(self, x0):
        
		x = self.relu(self.conv0(x0))
		x = self.relu(self.conv1(x))
		x = self.relu(self.conv2(x))
		x = self.relu(self.conv3(x))
		x = self.relu(self.conv4(x))
		x = self.relu(self.conv5(x))
		x = self.relu(self.conv6(x))
		x = self.conv7(x)

		return x+x0
  
    

# class Unmix(nn.Module):
#     def __init__(self, input_c, output_c):
#         super(Unmix, self).__init__()
#         self.net = nn.Sequential(
#             nn.Conv2d(input_c, output_c, 1, 1, 0, bias=False),
#         )
        
#     def forward(self, x):
#         return self.net(x).clamp_(0,1)

class Autoencoder(nn.Module):
    def __init__(self):
        super(Autoencoder, self).__init__()
        
        # 编码器
        self.net1 = nn.Sequential(
            nn.Conv2d(93, 64, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            nn.Conv2d(64, 16, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            # nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(16, 8, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            # nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(8, 3, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            nn.Sigmoid()
        )
        
        # 解码器
        self.net2 = nn.Sequential(
            nn.ConvTranspose2d(3, 8, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            nn.ConvTranspose2d(8, 16, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            nn.ConvTranspose2d(16, 64, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            nn.ConvTranspose2d(64, 93, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
        )


    def encoder(self, x):
        return self.net1(x)
    
    def decoder(self, x):
        x1 = self.net2(x)
        return x1
        
    def forward(self, x):
        # 编码
        x1 = self.net1(x)
        
        
        # 解码
        x = self.net2(x1)
        return x1,x