...
class GaussianDiffusion:
    ...
    def sample_loop_progressive(self,
        ...
        # E = th.eye(4).unsqueeze(0).to(device) 
        # print(E.shape)
        # exit()
        ...
        for i in indices:
            ...
            # loss = loss_func(xhat_1, LRHS, PAN ,4)
            norm1 = th.norm(LRHS - xhat_3) # ||Y - D(B(Ax_3 E + R))||
            loss = 0.1*loss_func(xhat_1, PAN)+2*norm1

            # xhat_4 = th.matmul((xhat_1).permute(0,2,3,1), param["PH"]).permute(0,3,1,2) # \hat{X}_3 r

            # g_path = '/home/jinliang/projects/mycode/dip/myINR/mat/dip.mat'
            # data_g = sio.loadmat(g_path)
            # fp = data_g['G']
            # fg = th.from_numpy(fp).unsqueeze(0)
            # p_3d = PAN.repeat(1,4,1,1)
            ...
            # # exit()
            # xhat_4 = fg * p_3d
            # norm2 = th.norm(xhat_1 - xhat_4) # ||P - \hat{X}_3 r||
