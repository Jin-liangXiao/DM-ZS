...
class GaussianDiffusion:
    ...
    def loss_tv(self, param, xhat):
        # regularization term
        weight_dx, weight_dy, weight_dz = 1, 1, 1
        xhat_dx, xhat_dy, xhat_dz = diff_3d(xhat)
        loss_2 = (th.norm(weight_dx * xhat_dx, p=1) +
                  th.norm(weight_dy * xhat_dy, p=1) +
                  th.norm(weight_dz * xhat_dz, p=1)
                  ) / xhat.numel()


        loss_condition = loss_2
    ...
    def sample_loop_progressive(self,
        ...
        for i in indices:
            ...
            baseA = (out["pred_xstart"] +1)/2
            E=E.to(th.float32)
            LRHS =LRHS.to(th.float32)
            PAN =PAN.to(th.float32)
            PAN = PAN.to(device)

            if label:
                net_input = LRHS.reshape(1, Cc, 64 * 64)
                im_out1 = AE(baseA).squeeze()
                im_out2 = AE2(net_input.unsqueeze(0)).squeeze(0)
                xhat_1 = th.tensordot(im_out2,im_out1, dims=([2], [0]))
            else:
                xhat_1 = th.matmul(E, baseA.reshape(Bb, Rr, -1)).reshape(*shape) + add_res
            xhat_2 = blur(xhat_1) 
            xhat_3 = down(xhat_2)
            net_target = AE_guide(LRHS,PAN)
            norm3 = th.norm(net_target - xhat_1)

            norm1 = th.norm(LRHS- xhat_3)
            RTZ = th.tensordot(xhat_1, param['PH'], dims=([1], [1])) 
            xhat_4=th.Tensor.permute(RTZ,(0,3,1,2))
            norm2 = th.norm(PAN- xhat_4)
            

            loss = norm1+0.1*norm3+norm2
            ...
            out["sample"] = out["sample"] - 2*norm_gradX #
